$(document).ready(function(){
	function e_passed() {
	    $("#jqgrid_passed").length > 0 && t_passed.fluidGrid({
	            base: "#jqgrid-wrapper_passed",
	            offset: 0
	        })
	    } 
	    var t_passed= $("#jqgrid_passed");
	    $("#jqgrid_passed").length > 0 && (t_passed.jqGrid({
	    	url:"myRenkanshuPassedList.action",
	    	mtype:"GET",
	    	datatype:"json",
			...
			loadComplete: function(data){
				var ids = $("#jqgrid_passed").jqGrid("getDataIDs");
				for(var i=0;i < ids.length;i++){
					console.log(ids[i]);
					var rowData = $("#jqgrid_passed").jqGrid('getRowData',ids[i]);
					...
					if("添加"==operType){
						...
						t_passed.jqGrid('setRowData',ids[i],{actions:myAdd_update});
					}else{
						update = '<button class="btn btn-primary btn-xs" data-target="'+ids[i]+'" onclick="myUpdateRenkanshuaudit(this)">修改</button>';
						t_passed.jqGrid('setRowData',ids[i],{actions:update});
					}
				}
                $("li#auditPassed a span").addClass("badge");
                $("li#auditPassed a span")[0].innerHTML = data.records;
			}
	    }), e_passed(), $("#jqgrid_passed").length > 0 && t_passed.jqGrid("navGrid","#jqgrid-pager_passed",{
	    	add:!1,
	    	edit:!1,
	    	del:!1,
	    	view:!1,
