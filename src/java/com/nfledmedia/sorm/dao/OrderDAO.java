...
public class OrderDAO extends HibernateDaoSupport {
	...
	public Order findById(java.lang.Integer id) {
		...
		try {
			Order instance = (Order) getHibernateTemplate().get(
					"com.nfledmedia.sorm.entity.Order", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public Page getRenkanshuManageList(String sidx, String sord, int pageNo, int pageSize) {
		...
		Page page = null;
		try {
			page = pagedQuery(GET_ORDER_LIST + " where o.state='"+TypeCollections.ORDER_STATE_ACTIVE+"' "
					+ " order by o." + sidx + " " + sord, pageNo, pageSize);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return page;
	...
	public Page getRenkanshuManageListByKeyword(String keyword, String sidx,
			String sord, int pageNo, int pageSize) {
		Page page = null;
		try {
			page = pagedQuery(GET_ORDER_LIST + keyword + " order by o."
	...
	public List<?> findOrdersOnDateByLed(java.sql.Date checkDate, Integer ledId) {
		...
		String hql = "select o from Order o where o.startdate <= '" + checkDate + "' and o.enddate >= '" + checkDate + "' and o.led = "+ ledId;
